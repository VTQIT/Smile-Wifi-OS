...
export default function App() {
  ...
  return (
    <div className="h-svh w-full bg-[#0a0a0c] text-white font-sans selection:bg-white/20 overflow-hidden relative flex flex-col">
      ...
      <div className="relative z-10 w-full max-w-md mx-auto h-full flex flex-col px-6 pt-safe pb-safe">
        {/* Header */}
        <header className="flex justify-start py-4 shrink-0">
          <button className="bg-black/40 backdrop-blur-md border border-white/10 px-4 py-1.5 rounded-md text-sm font-medium active:scale-95 transition-transform">
            Menu
          </button>
        </header>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <motion.div 
            ...
            className="relative aspect-video rounded-2xl overflow-hidden mb-6 shadow-2xl shadow-black/50 border border-white/5 bg-black shrink-0"
          >
            <video 
              autoPlay 
              muted 
          ...
          <div className="text-center mb-6 shrink-0">
            <h1 className="text-4xl font-bold tracking-tight text-white/90 mb-1 font-serif italic opacity-80">Cribble</h1>
            <p className="text-[9px] uppercase tracking-[0.2em] text-white/40 font-semibold leading-relaxed px-4">
          ...
          <div className="relative mb-8 shrink-0">
            <input
              ...
              className="w-full bg-white rounded-full py-3.5 px-6 text-black placeholder:text-gray-400 focus:outline-none shadow-lg text-base"
            />
            <Search className="absolute right-5 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" />
          </div>

          {/* App Grid with Swipe */}
          <div className="relative overflow-hidden flex-1 mb-4 min-h-0">
            <motion.div
              drag="x"
            ...
            >
              {Array.from({ length: totalPages }).map((_, pageIndex) => (
                <div key={pageIndex} className="min-w-full grid grid-cols-3 grid-rows-2 gap-y-6 gap-x-4 content-start">
                  {filteredApps
        ...
        {/* Footer Navigation */}
        <footer className="flex justify-between items-center px-8 py-4 shrink-0">
